...
@php
    ...
    $absenteesColl = collect($absentees);
@endphp
...
<div class="modal-content-wrapper text-right">
    ...
    <div class="em-stat-grid">
        <div class="em-stat-card danger">
            <span class="label">⚠️ الطلاب المتغيبين</span>
            <span class="value">{{ $absenteesColl->count() }}</span>
        </div>
        <div class="em-stat-card info">
            <span class="label">👥 إجمالي المرحلة</span>
            <span class="value">
                {{ $stats['total_stage_students'] ?? ($absenteesColl->count() + ($stats['attended_count'] ?? 0)) }}
            </span>
        </div>
        <div class="em-stat-card success">
            <span class="label">✅ الذين أدوا الامتحان</span>
            <span class="value">
    ...
    @if($absenteesColl->count() > 0)
        <div class="em-table-container">
            <table class="em-table">
                ...
                <tbody>
                    @foreach($absenteesColl as $index => $st)
                        @php
                            ...
                            $msg = "السلام عليكم ورحمة الله، المكرم ولي أمر الطالب/ة: {$name} ⚠️\n"
                                . "نحيطكم علماً بأن الطالب تغيب عن أداء امتحان ({$examTitle}) في مادة ({$subjectName}) المقررة بتاريخ: {$examDate}.\n"
    ...
    @else
        <div style="padding: 32px 16px; text-align: center; border-radius: 14px; border: 1px dashed #10b981; background: rgba(16, 185, 129, 0.08);">
            ...
            <h4 style="font-weight: 900; font-size: 15px; margin-bottom: 4px; color: #059669;">ممتاز! لا يوجد أي طلاب متغيبين عن هذا الامتحان</h4>
            <p style="font-size: 12px; opacity: 0.75; margin: 0;">جميع طلاب المرحلة المقيدين أتموا الامتحان بنجاح.</p>
        </div>
    @endif
</div>
